...
function apply(ctx) {
  ...
  /**
   * One `sessionPersistence.list()` entry as a `SessionHeader`, or null when it
   * carries no usable id. DSH 0.1.5 wraps every stored session in a
   * `SessionPersistenceSnapshot` ({ header, revision, eventCount?, sizeBytes? });
   * earlier versions return the headers themselves.
   */
  const headerOf = (entry) => {
    const header = entry?.header ?? entry
    return header !== null && typeof header?.id === 'string' ? header : null
  }

  /**
   * Title of one persisted session: its last `session/title` event. DSH 0.1.5
   * removed `readRaw` in favour of lifecycle handles (`open` + `read`), so use
   * typed events when the handle API exists and fall back to raw JSONL lines.
   */
  const readPersistedTitle = async (id) => {
    if (typeof persistence.open === 'function') {
      ...
      try {
        ...
        for (let i = events.length - 1; i >= 0; i -= 1) {
          ...
          if (event?.type === 'session/title' && typeof event.data?.title === 'string' && event.data.title.length > 0) return event.data.title
        }
        return ''
      } finally {
        try { await handle.release?.() } catch { /* handle disposal belongs to the persistence lifecycle */ }
      }
    }
    if (typeof persistence.readRaw !== 'function') return ''
  ...
  /**
   * Drop `undefined` properties from one session record. Tool output is
   * validated as lossless JSON, which rejects `undefined` values (the gateway
   * then fails the whole call with "value is not lossless JSON"); the schema
   * marks every field besides id/live/status optional, so omitting a key is
   * always valid.
   */
  const lossless = (record) => {
    const out = {}
  ...
  const listPersisted = async () => {
    ...
    try {
      ...
      const headers = (Array.isArray(entries) ? entries : []).map(headerOf).filter((header) => header !== null)
  ...
  /**
   * Last activity of one persisted session (cached 60s). File mtime needs
   * `locate`, which DSH 0.1.5 removed; snapshots expose no path, so fall back
   * to the session header's creation time.
   */
  ...
  /** Title of one persisted session: last `session/title` event in its log (cached). */
  const titleOfPersisted = async (header) => {
    ...
    let title = ''
    try {
      title = await readPersistedTitle(header.id)
   ...
   * Every field stays a defined primitive: the tool result must survive
   * lossless-JSON validation, which rejects `undefined` properties.
   */
  const describePersisted = async (header) => ({
    ...
    parent: typeof header.parentSession === 'string' ? header.parentSession : ''
  })
  ...
  ctx.tools.register(defineTool({
    ...
    async execute() {
      ...
      return [...live, ...offline].sort((a, b) => b.lastActive - a.lastActive).map(lossless)
    }
  }))
